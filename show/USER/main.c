...
u8 receive_flg=0; //接收数据标志，1表示接收到数据，0表示接收不到数据
void loopAll()
{ 
	u16 i;
	u8 reclen=0;  
...
//	u8 humi=0;
	...
	//	humi=(u8)humidity;
		...
		if(receive_flg==0)
		{
				OLED_ShowString(80,1,"no pkg",16);
				temperature=0;
				humidity=0;
		}
		else OLED_ShowString(80,1,"      ",16);
//		show_R(44,1,(float)temperature);
		OLED_ShowNum(44,1,temperature,3,16);
		delay_ms(600);	

//		show_R(44,4,(float)humidity);
		OLED_ShowNum(44,4,humidity,3,16);
